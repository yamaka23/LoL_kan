                    ...
                    <form action="{{ route('posts.update', $post) }}" method="POST">
                        ...
                        <div class="space-y-8" 
                            x-data="formController({
                                ...
                                initialData: {
                                    champion_id: '{{ old('champion_id', $post->champion_id) }}',
                                    vs_champion_id: '{{ old('vs_champion_id', $post->vs_champion_id) }}',
                                    mainPathId: {{ $mainPathRunes->isNotEmpty() ? $mainPathRunes->first()->rune_path_id : 'null' }},
                                    subPathId: {{ $subPathRunes->isNotEmpty() ? $subPathRunes->first()->rune_path_id : 'null' }},
                                    selectedMainRunes: {{ json_encode($selectedMainRunes) }},
                                    selectedSubRunes: {{ json_encode($selectedSubRunes) }},
                                    selectedStatRunes: {{ json_encode($selectedStatRunes) }},
                                    selectedItems: {{ json_encode($paddedItems) }}
                                }
                            })"
                            x-init="init()">
                                    ...
                                    <div>
                                        ...
                                        <select id="lane_id" name="lane_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:border-gray-700" required>
                                            ...
                                            @foreach ($lanes as $lane)
                                                <option value="{{ $lane->id }}" @selected(old('lane_id', $post->lane_id) == $lane->id)>{{ $lane->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
        ...
        function formController(data){
            return {
                ...
                init() {
                    ...
                    if (data.initialData) {
                        // チャンピオンを初期化
                        this.selectedChampionId = data.initialData.champion_id;
                        this.selectedVsChampionId = data.initialData.vs_champion_id;
                        
                        // ルーンを初期化
                        if (data.initialData.mainPathId) {
                            this.selectMainPath(data.initialData.mainPathId);
                            this.selectedMainRunes = data.initialData.selectedMainRunes;
                        }
                        if (data.initialData.subPathId) {
                            this.selectSubPath(data.initialData.subPathId);
                            this.selectedSubRunes = data.initialData.selectedSubRunes;
                        }
                        this.selectedStatRunes = data.initialData.selectedStatRunes;
                        
                        // アイテムを初期化
                        this.selectedItems = data.initialData.selectedItems;
                    }
                },
